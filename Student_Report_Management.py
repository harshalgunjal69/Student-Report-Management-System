...
try:
    cursor.execute("CREATE TRIGGER cgpa "
                   "BEFORE INSERT ON students FOR EACH ROW "
                   "SET NEW.CGPA = ((NEW.math_marks + NEW.phy_marks + NEW.chem_marks + "
                   "NEW.eng_marks + NEW.comp_marks)/5)/8 ;")
    mydb.commit()
except Exception as e:
    error_trigger_cgpa = e
#endregion
...
def display(table_headings, table_data):
    """
    Takes in the table headings and the table data in
    the form of list and prints a table out of it.
    """
    table = PrettyTable()
...
def view_all(_class):
    ...
    flag = 0
    try:
        qry = "SELECT roll_no, student_name, math_marks, phy_marks, chem_marks, " \
              "eng_marks, comp_marks, total, CGPA FROM students WHERE class = {} ORDER BY roll_no".format(_class)
        execute(qry)
        headings = ["Roll no.", "Student Name", "Maths Marks", "Physics Marks",
...
def check(_class, _rollno):
    ...
    try:
        qry = "SELECT roll_no, student_name, math_marks, phy_marks, chem_marks," \
              " eng_marks, comp_marks, total, CGPA FROM students WHERE class = {} AND roll_no = {}".format(_class, _rollno)
        execute(qry)
        data = cursor.fetchall()
        headings = ["Roll no.", "Student Name", "Maths Marks", "Physics Marks",
                    "Chemistry Marks", "English Marks", "Computer marks", "Total", "CGPA"]
...
def topper(_class):
    ...
    flag = 0
    try:
        qry = "SELECT RANK() over (ORDER BY CGPA DESC) Rankings, roll_no, student_name, " \
              "math_marks, phy_marks, chem_marks, eng_marks, comp_marks, total, CGPA " \
              "FROM students WHERE class = {} ORDER BY CGPA DESC LIMIT 3".format(_class)
        execute(qry)
        data = cursor.fetchall()
...
def _update(new, value, _class, _rollno):
    ...
    flag = 0
    try:
        qry = "UPDATE students SET {} = '{}' WHERE class = {} AND roll_no = {}".format(value, new, _class, _rollno)
        execute(qry)
        mydb.commit()
        if cursor.rowcount == 1:
            print("\nData updated successfully.")
        else:
            print("\nEither you've entered the same value as the previous value"
                  " or there is no such value present in the database. "
                  "Plz try again.")
    except Exception as e:
        print("\n", e)
        flag = 1
...
def delete(_class, _rollno):
    ...
    flag = 0
    try:
        qry = "DELETE FROM students WHERE class = {} AND roll_no = {}".format(_class, _rollno)
        execute(qry)
        mydb.commit()
        if cursor.rowcount == 1:
