...
def view_all(edu_class):
    ...
    try:
        qry = "SELECT roll_no, student_name, math_marks, phy_marks, chem_marks, " \
              "eng_marks, comp_marks, total, CGPA FROM students " + "WHERE class = "\
              + edu_class + " order by roll_no"
        execute(qry)
...
def topper(edu_class):
    ...
    try:
        qry = "SELECT RANK() over (ORDER BY CGPA DESC) Rankings, roll_no, student_name, " \
              "math_marks, phy_marks, chem_marks, eng_marks, comp_marks, total, CGPA FROM " \
              "students WHERE class = " + edu_class + " ORDER BY CGPA DESC LIMIT 3"
        execute(qry)
        data = cursor.fetchall()
...
def delete(_class, _rollno):
    ...
    flag = 0
    try:
        qry = "DELETE FROM students WHERE class = " + _class + " and roll_no = " + _rollno
        execute(qry)
        mydb.commit()
